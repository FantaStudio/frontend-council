...
const {
  readPersonaFile,
  buildMemberPrompt,
} = require('./lib/prompt-assembly.js');
const { enrichMembersWithMetadata } = require('./lib/member-metadata.js');
const {
  DESKTOP,
  CLI,
...
function resolveMemberModel(member) {
  if (member?.model) return String(member.model);
  const tier = member?.tier ? String(member.tier) : null;
  if (tier && TIER_MODELS[tier]) return TIER_MODELS[tier];
  return null;
}
...
function buildCouncilUiPayload(statusPayload) {
  ...
  const memberSteps = sortedMembers.map((m) => {
    ...
    const label = `[Frontend Council] Ask ${m.member}`;
  ...
  const codexPlan = [
    { step: `[Frontend Council] Prompt dispatch`, status: dispatchStatus },
    ...memberSteps.map((s) => ({ step: s.label, status: s.status })),
    { step: `[Frontend Council] Synthesize`, status: synthStatus },
  ];

  const claudeTodos = [
    {
      content: `[Frontend Council] Prompt dispatch`,
    ...
    {
      content: `[Frontend Council] Synthesize`,
      status: synthStatus,
      activeForm:
...
function computeStatusPayload(jobDir) {
  ...
  for (const m of members) {
    ...
    if (state === 'awaiting_desktop') {
      totals.awaiting_desktop++;
      totals.queued++;
      continue;
    }
...
Usage:
  council-job.sh start [--desktop|--cli] [--config path] [--chairman auto|claude|codex|...] [--jobs-dir path] [--json] "question"
  council-job.sh record [--stdin|--output-file path] [--state done|error] [--message text] <jobDir> <member> [output]
...
Notes:
  - desktop mode (default): prepares prompts; host runs Cursor Task subagents and records via record
  - cli mode: runs members in parallel via detached Node workers
...
function writeMemberPrompt({ skillDir, member, userQuestion, memberDir }) {
  const personaFile = member.persona_file ? String(member.persona_file) : null;
  if (!personaFile) {
    throw new Error(`member ${member.name}: missing persona_file in config`);
  }
  const personaContent = readPersonaFile(personaFile, skillDir);
  const memberPrompt = buildMemberPrompt({
    personaContent,
    userQuestion,
  });
  fs.writeFileSync(path.join(memberDir, 'prompt.txt'), memberPrompt, 'utf8');
}
...
function cmdStart(options, prompt) {
  ...
  const executionMode = resolveExecutionMode(config, options);
  const isDesktopExecution = executionMode === DESKTOP;

  const requestedMembers = config.council.members || [];
  const members = requestedMembers.filter((m) => {
    if (!m || !m.name) return false;
    if (isDesktopExecution) {
      if (!resolveMemberModel(m)) return false;
    } else if (!m.command) {
      return false;
    }
  ...
  if (members.length === 0) {
    exitWithError(
      isDesktopExecution
        ? 'start: no members with model/tier configured for desktop execution'
        : 'start: no members with command configured for cli execution',
    );
  }
  ...
  const jobMeta = {
    ...
    members: members.map((m) => ({
      name: String(m.name),
      command: !isDesktopExecution && m.command ? String(m.command) : null,
      ...
      tier: m.tier ? String(m.tier) : null,
      model: resolveMemberModel(m),
      execution: executionMode,
      persona_file: m.persona_file ? String(m.persona_file) : null,
    })),
  };
  ...
  for (const member of members) {
    ...
    const promptPath = path.join(memberDir, 'prompt.txt');
    const memberModel = resolveMemberModel(member);
    const initialState = isDesktopExecution ? 'awaiting_desktop' : 'queued';
    ...
    atomicWriteJson(path.join(memberDir, 'status.json'), {
      ...
      model: memberModel,
      ...(isDesktopExecution ? {} : { command: String(member.command) }),
    });
    ...
    if (isDesktopExecution) {
      desktopMembers.push({
        name,
        model: memberModel,
        level: member.level ? String(member.level) : null,
        project: member.project ? String(member.project) : null,
        tier: member.tier ? String(member.tier) : null,
        promptPath,
  ...
  if (options.json) {
    const payload = {
      jobDir,
      execution: executionMode,
      ...(isDesktopExecution ? { desktopMembers } : {}),
      ...jobMeta,
    };
...
function cmdResults(options, jobDir) {
  ...
  if (options.json) {
    const jobMembers =
      jobMeta && Array.isArray(jobMeta.members) ? jobMeta.members : [];
    const enrichedMembers = enrichMembersWithMetadata(
      members.map((m) => ({
        member: m.member,
        state: m.state,
        exitCode: m.exitCode != null ? m.exitCode : null,
...
function findMemberSafeName(jobDir, memberName) {
  const membersRoot = path.join(jobDir, 'members');
  if (!fs.existsSync(membersRoot)) {
    exitWithError(`record: members folder not found: ${membersRoot}`);
  }

  const target = String(memberName || '')
    .trim()
    .toLowerCase();
  if (!target) exitWithError('record: missing member name');

  for (const entry of fs.readdirSync(membersRoot)) {
    const statusPath = path.join(membersRoot, entry, 'status.json');
    const status = readJsonIfExists(statusPath);
    if (status && String(status.member).toLowerCase() === target) {
      return entry;
    }
  }

  exitWithError(`record: member not found: ${memberName}`);
}

function cmdRecord(options, jobDir, memberName, trailingOutput) {
  const resolvedJobDir = path.resolve(jobDir);
  const safeName = findMemberSafeName(resolvedJobDir, memberName);
  const memberDir = path.join(resolvedJobDir, 'members', safeName);
  const statusPath = path.join(memberDir, 'status.json');
  const status = readJsonIfExists(statusPath);
  if (!status) exitWithError(`record: status.json missing for ${memberName}`);

  let outputText = '';
  if (options.stdin) {
    outputText = fs.readFileSync(0, 'utf8');
  } else if (options['output-file']) {
    outputText = fs.readFileSync(
      path.resolve(String(options['output-file'])),
      'utf8',
    );
  } else if (trailingOutput) {
    outputText = String(trailingOutput);
  } else {
    exitWithError(
      'record: provide output via --stdin, --output-file, or trailing argument',
    );
  }

  const stateRaw = options.state != null ? String(options.state) : null;
  const isError = Boolean(options.error) || stateRaw === 'error';
  const finishedAt = new Date().toISOString();

  if (isError) {
    fs.writeFileSync(path.join(memberDir, 'error.txt'), outputText, 'utf8');
    atomicWriteJson(statusPath, {
      ...status,
      state: 'error',
      finishedAt,
      exitCode: options['exit-code'] != null ? Number(options['exit-code']) : 1,
      message: options.message ? String(options.message) : null,
    });
  } else {
    fs.writeFileSync(path.join(memberDir, 'output.txt'), outputText, 'utf8');
    atomicWriteJson(statusPath, {
      ...status,
      state: 'done',
      finishedAt,
  ...
  process.stdout.write(
    `record: ${status.member} -> ${isError ? 'error' : 'done'}\n`,
  );
}
...
function main() {
  ...
  if (command === 'record') {
    const jobDir = rest[0];
    const memberName = rest[1];
    const trailingOutput = rest.slice(2).join(' ').trim() || null;
    if (!jobDir) exitWithError('record: missing jobDir');
    if (!memberName) exitWithError('record: missing member');
    cmdRecord(options, jobDir, memberName, trailingOutput);
    return;
  }
  if (command === 'status') {
    const jobDir = rest[0];
