...
#   council.sh record [options] JOB_DIR member  # desktop: save Task output
...
Usage:
  $(basename "$0") start [--desktop|--cli] [options] "question"
  $(basename "$0") record [--stdin|--output-file path] [--state done|error] <jobDir> <member> [output]
...
case "$1" in
  start|status|wait|results|stop|clean|record)
    exec "$JOB_SCRIPT" "$@"
    ;;
esac
